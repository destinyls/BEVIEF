...
category_map = {'Car': 'Car', 'Van': 'Car', 'Truck': 'Car', 'Bus': 'Car', 'Pedestrian': 'Pedestrian', 'Cyclist': 'Cyclist', 'Bicycle': 'Cyclist', 'Motorcycle':'Cyclist','Motorcyclist': 'Cyclist', 'Tricyclist': 'Cyclist', 'Trafficcone': 'Trafficcone', 'Unknown_unmovable': 'Unknown_unmovable', 'Barrow': 'Cyclist', 'Barrowlist':'Cyclist', 'Unknowns_movable': 'Unknowns_movable', 'Triangle plate': 'Triangle plate'}
...
def convert_label(src_label_file, dest_label_file):
    ...
    for line in lines:
        ...
        if np.sum(pos) < 10e-9 or label[0] not in category_map.keys():
            continue
